...
async function checkTablesWithoutRLS(ref: string, pat: string): Promise<LintResult> {
    ...
    return {
        findings: [{
            ...
            recommendation: "Enable RLS on each table: ALTER TABLE public.{table} ENABLE ROW LEVEL SECURITY; Then add appropriate policies.",
            evidence: tables,
        }],
...
async function checkDangerousExtensions(ref: string, pat: string): Promise<LintResult> {
    ...
    return {
        findings: [{
            ...
            recommendation: "Drop these extensions if not actively needed: DROP EXTENSION {ext};",
            evidence: exts,
        }],
