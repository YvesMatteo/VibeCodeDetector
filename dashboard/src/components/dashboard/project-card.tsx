...
export function ProjectCard({ id, name, url, faviconUrl, issueCount = 0, severity, lastAuditDate }: ProjectCardProps) {
    ...
    return (
        <Link href={`/dashboard/projects/${id}`} className="group block h-full">
            <div className="relative h-full rounded-xl border border-white/[0.06] bg-white/[0.01] px-5 py-5 flex flex-col transition-all duration-200 hover:border-white/[0.1] hover:bg-white/[0.03]">
                {/* Top: favicon + name + time */}
                <div className="flex items-start gap-3 mb-6">
                    {imgError ? (
                        <div className="h-9 w-9 rounded-lg bg-zinc-800/50 flex items-center justify-center shrink-0 border border-white/[0.08]">
                        ...
                        <img
                            src={faviconSrc}
                            alt=""
                            className="h-9 w-9 rounded-lg object-contain shrink-0 p-1 bg-zinc-900/50 border border-white/10 group-hover:border-white/20 transition-colors"
                    ...
                    <div className="min-w-0 flex-1 py-0.5">
                        ...
                        <p className="text-[12px] text-zinc-500 truncate mt-0.5">{hostname}</p>
                    </div>
                    {hasAudit && (
                        <div className="flex items-center gap-1.5 text-[11px] text-zinc-500 shrink-0 mt-1">
                    ...
                    {hasAudit ? (
                        <>
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-sm font-semibold text-zinc-300 tabular-nums">{issueCount}</span>
                                <span className="text-[12px] text-zinc-500">issues found</span>
                                {issueCount > 0 && (
                        ...
                        <p className="text-[12px] text-zinc-500">No audits yet</p>
                    )}
                </div>
            </div>
        </Link>
