...
export function ProjectCard({ id, name, url, issueCount = 0, severity, lastAuditDate }: ProjectCardProps) {
    ...
    return (
        <Link href={`/dashboard/projects/${id}`} className="group block">
            <div className="relative rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 transition-all duration-200 hover:border-white/[0.12] hover:bg-white/[0.03]">
                ...
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className="min-w-0 flex-1">
                        <h3 className="text-[14px] font-medium text-zinc-200 truncate group-hover:text-white transition-colors">
                            {name}
                        </h3>
                        <p className="text-[11px] text-zinc-600 truncate">{hostname}</p>
                    </div>
                    {hasAudit && (
                ...
                {hasAudit ? (
                    <>
                        {/* Severity mini-bar */}
                        {issueCount > 0 && (
                            <div className="flex h-1 rounded-full overflow-hidden bg-white/[0.04] mb-3">
                        ...
                        <div className="flex items-center gap-3">
                            <span className="text-[13px] font-semibold text-white tabular-nums">{issueCount}</span>
                            <span className="text-[11px] text-zinc-600">{issueCount === 1 ? 'issue' : 'issues'}</span>
                            {issueCount > 0 && (
