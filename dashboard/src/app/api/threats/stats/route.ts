...
export async function GET(req: NextRequest) {
    ...
    // Get aggregated stats via RPC (custom RPC not in generated types)
    const { data: stats, error: statsError } = await (supabase
        .rpc as unknown as (fn: string, params: Record<string, unknown>) => Promise<{ data: unknown; error: unknown }>)
        ('get_threat_stats', { p_project_id: projectId, p_since: since });
    ...
    // Get hourly time-series for chart (threat_events not in generated types)
    const { data: timeSeries, error: tsError } = await (supabase
        .from('threat_events' as never)
        .select('created_at, severity')
        .eq('project_id' as never, projectId)
        .gte('created_at' as never, since)
        .order('created_at', { ascending: true })) as unknown as { data: ThreatEvent[] | null; error: unknown };
    ...
    const { data: topIpsRaw } = await (supabase
        .rpc as unknown as (fn: string, params: Record<string, unknown>) => Promise<{ data: unknown; error: unknown }>)
