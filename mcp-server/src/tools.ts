...
/** Hostnames and IP patterns that must never be scanned (SSRF defense-in-depth) */
...
function isPrivateIP(hostname: string): boolean {
  ...
  if (h === '::1' || h === '::' || h.startsWith('fe80:') || h.startsWith('fc00:') || h.startsWith('fd')) return true;
...
function isBlockedUrl(urlStr: string): boolean {
  try {
    ...
    if (BLOCKED_HOSTNAMES.has(parsed.hostname.toLowerCase())) return true;
...
export function registerTools(server: McpServer, client: CheckVibeClient) {
  server.tool(
    ...
    'Trigger a new security scan on a URL. Returns the scan ID, overall score, and a summary of findings by severity.',
    {
      ...
      github_repo: z.string().url().optional().describe('GitHub repository URL for deep secret scanning'),
      supabase_url: z.string().url().optional().describe('Supabase project URL for backend scanning'),
    },
    async ({ url, github_repo, supabase_url }) => {
      try {
        ...
        if (!['http:', 'https:'].includes(parsed.protocol)) {
          return errorResponse('Only http and https URLs are allowed.');
        }

        // SSRF defense-in-depth: block private/internal targets
        if (isBlockedUrl(targetUrl)) {
          return errorResponse('Cannot scan private, internal, or localhost URLs.');
        }

        // Use the validated, normalized URL for the API call
        const result = await client.runScan(targetUrl, github_repo, supabase_url);
    ...
    async ({ scan_id }) => {
      try {
        ...
        if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(scan_id)) {
          return errorResponse('Invalid scan ID format. Expected a UUID.');
        }
        ...
        if (scan.results) {
          for (const [scannerName, result] of Object.entries(scan.results)) {
            ...
            if (result.error) {
              lines.push(`  Error: Scanner encountered an issue`);
            }
